        ...
        <div className="overflow-x-auto border border-gray-200 dark:border-[#2B3139] rounded-2xl mb-6">
          <table className="w-full text-left border-collapse min-w-[500px]">
            ...
            <tbody className="text-[17.5px]">
              {rows.map((r, i) => (
                <tr key={i} className="border-b border-gray-100 dark:border-[#2B3139]/50 last:border-0 active:bg- md:hover:bg-white dark:active:bg- md:hover:bg-[#1A2639]/30">
...
const PatternCard = ({ svgCandles, name, type, badgeClass, essence, steps, isWarning }) => {
  ...
  return (
    <div className="border border-gray-200 dark:border-[#2B3139] rounded-3xl overflow-hidden mb-6 transition-all shadow-sm active:shadow- md:hover:shadow-md dark:shadow-none dark:active:shadow- md:hover:shadow-[0_0_20px_rgba(0,0,0,0.5)] bg-white dark:bg-[#181A20]">
      <div onClick={() => setIsOpen(!isOpen)} className="flex items-center justify-between p-6 md:p-8 cursor-pointer active:bg- md:hover:bg-gray-50 dark:active:bg- md:hover:bg-[#2B3139]/50 transition-colors">
      ...
      {isOpen && (
        <div className="p-6 md:p-8 bg-gray-50 dark:bg-[#0B0E11] border-t border-gray-200 dark:border-[#2B3139] animate-in fade-in transition-colors">
          <div className="bg-gray-100 dark:bg-[#1A2639]/50 border border-gray-200 dark:border-[#2B3139] rounded-2xl p-6 mb-6">
            <div className="text-[13.5px] font-mono font-black tracking-[0.15em] text-yellow-600 dark:text-[#FCD535] mb-3 uppercase">Bản chất</div>
...
const FinalQuizCh3 = () => {
  const qs = [
    { q: 'Nến thân ngắn xuất hiện tại vùng kháng cự $2,050 vàng sau uptrend. D1 đang uptrend. Bạn đặt Buy Stop H+10 và Sell Stop L-10. Giá bứt phá xuống dưới L và kích hoạt Sell Stop. Bạn làm gì tiếp theo?', opts: ['Giữ cả hai lệnh', 'Hủy Buy Stop ngay, giữ Sell Stop và quản lý lệnh', 'Đóng Sell Stop vì xu hướng D1 vẫn tăng', 'Thêm vào Sell Stop'], c: 1, exp: 'Theo quy tắc NNN①, khi một trong hai lệnh Stop được kích hoạt (ở đây là Sell Stop), bạn phải HỦY NGAY lệnh Stop còn lại (Buy Stop) để tránh bị dính cả hai đầu nếu giá giật.' },
    { q: 'Điều kiện CHÍNH XÁC của nến ôm (NNN②) là gì?', opts: ['Thân nến sau nhỏ hơn thân nến trước', 'H_mẹ ≥ H_con VÀ L_mẹ ≤ L_con (so sánh H và L tổng)', 'Nến sau màu ngược nến trước', 'Nến sau đóng cửa trong thân nến trước'], c: 1, exp: 'Định nghĩa chuẩn của Nến Ôm (Inside Bar) là toàn bộ nến con (cả thân và bóng) phải nằm gọn trong biên độ High-Low của nến mẹ.' },
    { q: 'EMA21 đang dốc lên. Giá pullback về test EMA21 rồi xuất hiện Dragonfly Doji bật lên. Đây là setup gì theo NNN?', opts: ['Chỉ là NNN① (nến thân ngắn)', 'Chỉ là NNN③ (EMA21)', 'NNN① + NNN③ đồng thời hội tụ = tín hiệu mạnh hơn nhiều', 'Không có setup vì Doji không đủ mạnh'], c: 2, exp: 'Đây là sự hội tụ (Confluence). Nến thân ngắn (NNN①) kết hợp với EMA21 (NNN③) tại cùng một điểm tạo ra một setup xác suất thắng cao hơn nhiều so với việc chỉ dùng 1 tín hiệu.' },
    { q: 'Giá pullback về Fibonacci 62% và xuất hiện Bullish Engulfing. R:R theo hệ NNN là bao nhiêu (TP1 = 127%, SL = dưới Swing Low)?', opts: ['Tùy thuộc vào Swing Low và Swing High cụ thể', 'Luôn là 1:2', 'Luôn là 1:3', 'R:R không quan trọng với NNN'], c: 0, exp: 'Tỷ lệ R:R phụ thuộc vào khoảng cách thực tế giữa Entry (ở mức 62%), SL (dưới 100% Swing Low) và TP (127%). Nó thay đổi tùy thuộc vào cấu trúc sóng cụ thể trên chart.' },
    { q: 'Morning Star xuất hiện ở đâu thì có giá trị nhất?', opts: ['Bất cứ đâu trong chart', 'Sau uptrend tại vùng kháng cự', 'Sau downtrend tại vùng hỗ trợ — đặc biệt trùng Fibonacci 62/79%', 'Giữa sideway range'], c: 2, exp: 'Mô hình đảo chiều Sao Mai (Morning Star) phát huy sức mạnh cao nhất khi nó xuất hiện sau một xu hướng giảm rõ ràng và tại một vùng hỗ trợ mạnh (ví dụ: Fibo 62% hoặc 79%).' },
    { q: 'Tại sao thầy NNN dùng mức 88% trong Fibonacci mặc dù nó gần 100% (Swing Low)?', opts: ['88% là mức Fibonacci tiêu chuẩn', 'Để có SL rộng hơn giúp không bị stopout khi giá về sâu 88% trước khi bật lên', '88% không quan trọng', 'Để có TP lớn hơn'], c: 1, exp: 'Nhiều trường hợp giá "quét" thanh khoản rất sâu trước khi đảo chiều. Mức 88% đóng vai trò là cơ hội vào lệnh cuối cùng hoặc cảnh báo sớm trước khi Swing Low (100%) bị phá vỡ.' },
    { q: 'Bullish Engulfing xuất hiện giữa uptrend, không gần S/R nào. Đây là tín hiệu gì?', opts: ['BUY mạnh — Engulfing luôn là tín hiệu tốt', 'Không đáng tin — Engulfing cần bối cảnh đúng (sau downtrend/tại S/R)', 'SELL — đây là đỉnh', 'Tiếp tục uptrend, mua thêm'], c: 1, exp: 'Mô hình nến cần bối cảnh (Context). Một Bullish Engulfing lơ lửng giữa chu kỳ tăng, không có sự hỗ trợ của S/R, thường thiếu độ tin cậy và dễ bị thất bại.' },
    { q: 'Bạn thấy: D1 uptrend + Fibonacci 62% + EMA21 hội tụ tại cùng một vùng + Hammer + RSI divergence bullish. Đây là bao nhiêu tín hiệu hội tụ?', opts: ['2 tín hiệu', '3 tín hiệu', '4 tín hiệu', '5 tín hiệu — Setup xếp hạng A++, xứng đáng size lớn hơn'], c: 3, exp: 'D1 (1) + Fibo 62% (2) + EMA21 (3) + Hammer (4) + RSI Divergence (5) = 5 yếu tố hội tụ. Đây là một Setup chất lượng rất cao.' },
    { q: 'Three White Soldiers xuất hiện sau Morning Star ở đáy. RSI đang ở 78. Bạn nên BUY không?', opts: ['BUY full size — 3 nến xanh = momentum cực mạnh', 'Thận trọng — RSI 78 (overbought). Chờ pullback về EMA21 thay vì đuổi theo giá đã tăng xa', 'Không bao giờ BUY khi RSI >70', 'SELL vì RSI overbought'], c: 1, exp: 'Mặc dù hành vi giá rất bullish, nhưng RSI 78 cho thấy giá đã tăng quá nhanh. Việc FOMO mua đuổi lúc này là rủi ro. Nên kiên nhẫn chờ giá hồi về EMA21.' },
    { q: 'Trong NNN④, sau khi giá đạt TP1 (127%), bước TIẾP THEO là gì?', opts: ['Đóng toàn bộ lệnh', 'Chốt 50% vị thế tại TP1, dời SL phần còn lại về điểm entry (Break Even)', 'Thêm vào vị thế vì giá đang mạnh', 'Dời SL về 79% Fibonacci'], c: 1, exp: 'Quy tắc quản lý lệnh chuẩn của NNN: Chốt một nửa lợi nhuận ở TP1 để bảo vệ thành quả và loại bỏ rủi ro bằng cách dời SL phần còn lại về điểm hòa vốn (Entry).' },
    { q: 'Shooting Star xuất hiện tại đỉnh uptrend, kháng cự $2,100. Bóng trên = 120 pip, thân = 30 pip. Bạn vào Sell Stop tại L − 10pip. SL đặt ở đâu?', opts: ['Dưới thân nến − 10pip', 'Trên H của Shooting Star + 10pip ($2,100 + bóng trên + 10)', 'Tại đường EMA21', 'Tại mức 50% Fibonacci'], c: 1, exp: 'Stop Loss luôn được đặt trên râu nến (High) của tín hiệu đảo chiều giảm cộng thêm một khoảng đệm (10 pips) để tránh bị quét bởi những cú rướn giá ngẫu nhiên.' },
    { q: 'Cặp EUR/USD H4: EMA21 dốc lên, giá vừa đóng cửa DƯỚI EMA21 lần đầu tiên sau 2 tuần. D1 vẫn uptrend. Đây là tín hiệu gì?', opts: ['Tín hiệu SELL — giá đã xuyên phá EMA21', 'CHoCH cảnh báo sớm — không mở Buy mới, xem xét thắt chặt SL của Buy cũ', 'Tín hiệu mua thêm vì D1 vẫn tăng', 'Không có ý nghĩa gì'], c: 1, exp: 'Giá đóng cửa dưới EMA21 sau một xu hướng tăng dài là dấu hiệu suy yếu đầu tiên. Tuy chưa đủ để SELL ngược D1, nhưng là tín hiệu cần ngưng BUY mới và bảo vệ lợi nhuận hiện có.' },
    { q: 'Harami Bearish xuất hiện ở đỉnh uptrend. Bạn nên làm gì NGAY LẬP TỨC?', opts: ['SELL full size ngay', 'Đóng 50% lệnh Buy đang giữ (chốt lời bảo toàn), chờ xác nhận thêm trước khi SELL hoàn toàn', 'Không làm gì — Harami yếu', 'Thêm vào Buy'], c: 1, exp: 'Harami là mô hình thể hiện sự do dự, không mạnh bằng Engulfing. Hành động khôn ngoan là chốt lời một phần lệnh BUY hiện có, chưa vội SELL vì chưa có nến xác nhận giảm mạnh.' },
    { q: 'Fibonacci NNN④ dùng mức 127% và 162% để làm gì?', opts: ['Xác định vùng hỗ trợ', 'TP1 (127% — chốt 50%) và TP2 (162% — chốt hết) cho lệnh đã vào từ vùng Retracement 62-88%', 'Entry điểm thứ 2', 'Stop Loss'], c: 1, exp: 'Các mức Fibonacci Mở rộng (Extension) 127% và 162% được sử dụng làm mục tiêu chốt lời (Take Profit) dự kiến cho các đợt sóng thuận xu hướng.' },
    { q: 'NGUYÊN TẮC VÀNG quan trọng nhất của hệ thống NNN là gì?', opts: ['Luôn dùng EMA21', 'Confluence: chỉ vào lệnh khi ≥3 tín hiệu NNN/pattern cùng hội tụ một vùng giá — không bao giờ vào lệnh chỉ vì 1 tín hiệu', 'Fibonacci mức 62% là entry tốt nhất', 'Luôn đặt SL 10pip'], c: 1, exp: 'Sự hội tụ (Confluence) là chìa khóa. Việc kết hợp nhiều yếu tố ủng hộ sẽ tăng xác suất chiến thắng của bạn lên mức cao nhất, lọc bớt các tín hiệu nhiễu.' }
  ];
    ...
    <div className="mt-10">
      ...
      {!showRes ? (
        ...
        <div className="p-10 bg-gradient-to-br from-white to-gray-50 dark:from-[#0B0E11] dark:to-[#181A20] border border-yellow-500 dark:border-[#FCD535] rounded-3xl text-center shadow-lg mt-8">
          ...
          <p className="text-gray-600 dark:text-[#848E9C] text-[18.5px] mb-6">{score >= 12 ? <><Trophy size={18} className="inline mr-1 text-yellow-500" /> Xuất sắc! Bạn đã sẵn sàng học Quản lý Vốn.</> : <><Dumbbell size={48} className="mx-auto text-blue-500 mb-4" /> Chưa đạt 12/15. Hãy đọc lại phần NNN Methods và Confluence trước.</>}</p>
...
const NNNChart1 = () => (
  <div className="bg-[#050c0f] border border-gray-800 rounded-2xl p-4 my-8 overflow-x-auto">
    <div className="text-[13.5px] text-gray-400 font-mono uppercase tracking-widest mb-3">// NNN① Minh họa — Nến thân ngắn (Doji) tại Support · Buy Stop &amp; Sell Stop</div>
...
const NNNChart2 = () => (
  <div className="bg-[#050c0f] border border-gray-800 rounded-2xl p-4 my-8 overflow-x-auto">
    <div className="text-[13.5px] text-gray-400 font-mono uppercase tracking-widest mb-3">// NNN② Minh họa — Nến ôm (Inside Bar) · Nến mẹ ôm trọn nến con</div>
...
const NNNChart3 = () => (
  <div className="bg-[#050c0f] border border-gray-800 rounded-2xl p-4 my-8 overflow-x-auto">
    <div className="text-[13.5px] text-gray-400 font-mono uppercase tracking-widest mb-3">// NNN③ Minh họa — Nến đóng cửa trên EMA21 → BUY Setup + Pullback Entry</div>
...
const NNNChart4 = () => {
  ...
  return (
    <div className="bg-[#050c0f] border border-gray-800 rounded-2xl p-4 my-8 overflow-x-auto">
      <div className="text-[13.5px] text-gray-400 font-mono uppercase tracking-widest mb-3">// NNN④ Minh họa — Fibonacci NNN: Swing Low→High → Pullback 62% → Entry → TP1/TP2</div>
  ...
  },
  {
    ...
    content: (
      <>
        ...
        <p className="text-[18.5px] leading-[1.8] text-gray-800 dark:text-[#EAECEF] mb-4"><strong>Định nghĩa chính xác từ vở NNN:</strong> nến thân ngắn là nến có Close gần bằng Open, thân nến rất nhỏ so với toàn bộ biên độ High-Low.</p>
  ...
  },
  {
    ...
    content: (
      <>
        ...
        <p className="text-[18.5px] leading-[1.8] text-gray-800 dark:text-[#EAECEF] mb-6"><strong>Định nghĩa chính xác:</strong> Cụm 2 cây nến liên tiếp, trong đó <em>H và L của cây nến trước (nến mẹ) "ôm trọn" H và L của cây nến sau (nến con).</em></p>
  ...
  },
  {
    ...
    content: (
      <>
        ...
        <p className="text-[18.5px] leading-[1.8] text-gray-800 dark:text-[#EAECEF] mb-6">Theo cách ghi của NNN, EMA21 là đường trung bình theo giá đóng cửa của 21 cây nến gần nhất. Điều kiện quan trọng là <strong>nến đóng cửa</strong> rõ ràng trên/dưới EMA21, không phải chỉ bóng nến chạm hoặc xuyên qua.</p>
        <NNNChart3 />
...
export default CHAPTER_3_DATA_VN;
