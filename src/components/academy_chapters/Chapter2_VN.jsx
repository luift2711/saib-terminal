...
const SRChart = () => {
  ...
  return (
    <div className="bg-gray-900 dark:bg-[#0A0D13] border border-gray-700 dark:border-[#2B3139] rounded-2xl p-6 my-8">
      <div className="text-[14.5px] text-gray-400 font-mono uppercase tracking-widest mb-4">// XAU/USD — Nhấp vào các vùng màu sáng để xem giải thích</div>
...
const VolumeChart = () => {
  ...
  return (
    <div className="bg-gray-900 dark:bg-[#0A0D13] border border-gray-700 dark:border-[#2B3139] rounded-2xl p-6 my-8">
      <div className="text-[14.5px] text-gray-400 font-mono uppercase tracking-widest mb-4">// Biểu đồ giá + Volume — Nhấp vào cột volume để xem phân tích</div>
...
const DivergenceDemo = () => (
  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 my-8">
    <div className="bg-white dark:bg-[#181A20] border border-green-500 dark:border-[#0ECB81] rounded-2xl p-6 shadow-lg">
      ...
      <p className="text-[17.5px] text-gray-700 dark:text-[#EAECEF] mb-6">Giá tạo đáy <strong className="text-black dark:text-white">thấp hơn</strong> nhưng RSI tạo đáy <strong className="text-green-600 dark:text-[#0ECB81]">cao hơn</strong>. Phe bán yếu dần. Cảnh báo đảo chiều TĂNG.</p>
    ...
    <div className="bg-white dark:bg-[#181A20] border border-red-500 dark:border-[#F6465D] rounded-2xl p-6 shadow-lg">
      ...
      <p className="text-[17.5px] text-gray-700 dark:text-[#EAECEF] mb-6">Giá tạo đỉnh <strong className="text-black dark:text-white">cao hơn</strong> nhưng RSI tạo đỉnh <strong className="text-red-600 dark:text-[#F6465D]">thấp hơn</strong>. Phe mua yếu dần. Cảnh báo đảo chiều GIẢM.</p>
      <div className="bg-gray-900 dark:bg-[#0A0D13] p-4 rounded-xl">
...
const MACDChart = () => (
  <div className="bg-gray-900 dark:bg-[#0A0D13] border border-gray-700 dark:border-[#2B3139] rounded-3xl p-6 my-8 shadow-xl">
    <div className="text-[14.5px] text-gray-400 font-mono uppercase tracking-widest mb-6">// MACD Minh họa — Crossover</div>
...
const DCACalculator = () => {
  ...
  return (
    <div className="bg-white dark:bg-[#181A20] border border-gray-200 dark:border-[#2B3139] rounded-3xl p-8 my-8 shadow-lg transition-colors">
        ...
        <table className="w-full text-left border-collapse min-w-[400px] text-[15.5px] border border-gray-200 dark:border-[#2B3139] rounded-xl overflow-hidden">
          ...
          <tbody className="text-[15.5px]">
            {Array.from({ length: Math.min(months, 24) }, (_, i) => {
              ...
              return (
                <tr key={m} className={`border-b border-gray-100 dark:border-[#2B3139]/50 last:border-0 ${isLast ? 'bg-green-50 dark:bg-[#0ECB81]/10 font-bold' : 'active:bg- md:hover:bg-gray-50 dark:active:bg- md:hover:bg-[#1A2639]/20'
                  }`}>
      ...
      <div className="text-center text-[16.5px] text-gray-600 dark:text-[#848E9C] mt-4">DCA là chìa khóa kỷ luật giúp người mới sống sót và chiến thắng tâm lý.</div>
    </div>
  );
};
...
const ResourceCard = ({ type, name, lang, desc, why, link }) => {
  ...
  return (
    <div className="border border-gray-200 dark:border-[#2B3139] rounded-2xl overflow-hidden mb-4 bg-white dark:bg-[#181A20] shadow-sm active:shadow- md:hover:shadow-md dark:shadow-none transition-all">
      <div onClick={() => setOpen(!open)} className="p-5 flex gap-4 items-center cursor-pointer active:bg- md:hover:bg-gray-50 dark:active:bg- md:hover:bg-[#2B3139]/40">
...
const FinalQuizCh2 = () => {
  const qs = [
    { q: 'Vùng kháng cự $2,000 vàng bị phá và giá đóng cửa ở $2,010. Hôm sau giá pullback về $2,000. Đây là cơ hội gì?', opts: ['Cơ hội SELL vì đây là kháng cự cũ', 'Cơ hội BUY theo Role Reversal — $2,000 vừa trở thành hỗ trợ mới', 'Đứng ngoài vì không chắc chắn', 'Chờ giá về $1,980 mới mua'], c: 1, exp: 'Theo nguyên tắc Role Reversal (Đổi vai), một khi kháng cự bị phá vỡ rõ ràng, nó sẽ trở thành hỗ trợ mới. Khi giá quay lại test vùng này, đó là cơ hội BUY.' },
    { q: 'Xu hướng BTC D1 đang downtrend (LH-LL). Trên H1 có setup BUY đẹp với RSI 28. Bạn nên làm gì?', opts: ['BUY theo H1 vì RSI oversold rất mạnh', 'Không BUY — D1 downtrend = không giao dịch ngược chiều D1', 'SELL theo D1 nếu H1 cho tín hiệu SELL', 'Chờ thêm 1 tuần'], c: 1, exp: 'Luôn tôn trọng xu hướng của khung thời gian lớn (D1). Nếu D1 đang giảm, bạn không nên BUY trên H1 dù RSI có oversold, vì đó là giao dịch ngược xu hướng rủi ro cao.' },
    { q: 'EMA21 > EMA50 > EMA200 và cả 3 đều dốc lên. Tình trạng thị trường là gì?', opts: ['Sideway mạnh', 'Strong Downtrend', 'Strong Uptrend — chỉ tìm cơ hội BUY', 'Chuyển đổi xu hướng'], c: 2, exp: 'Sự sắp xếp trật tự các đường EMA (nhỏ > trung bình > lớn) và cùng dốc lên là dấu hiệu rõ ràng của một xu hướng tăng mạnh (Strong Uptrend).' },
    { q: 'RSI = 72 sau khi giá tăng liên tiếp 5 ngày trong strong uptrend. Bạn nên làm gì?', opts: ['SELL ngay vì RSI overbought', 'BUY vào vì trong strong uptrend RSI 70+ là bình thường', 'Đứng ngoài hoàn toàn', 'Đặt lệnh SELL pending'], c: 1, exp: 'Trong một xu hướng tăng mạnh, RSI có thể nằm trong vùng overbought (>70) rất lâu. Bạn không nên SELL ngược xu hướng chỉ vì RSI > 70. Chờ giá pullback để BUY thuận xu hướng.' },
    { q: 'Giá EUR/USD tạo đáy mới thấp hơn ($1.05) nhưng RSI tạo đáy cao hơn (29 vs 22 trước). Đây là gì?', opts: ['Bearish Divergence — nên SELL', 'Bullish RSI Divergence — cảnh báo đảo chiều tăng', 'Không có ý nghĩa', 'RSI bị lỗi'], c: 1, exp: 'Giá tạo đáy thấp hơn nhưng chỉ báo tạo đáy cao hơn gọi là Phân kỳ tăng (Bullish Divergence), cảnh báo phe bán đang yếu dần và có thể sắp đảo chiều tăng.' },
    { q: 'Volume trong một phiên breakout = 80% trung bình 20 ngày. Bạn đánh giá breakout này thế nào?', opts: ['Breakout mạnh, tin tưởng cao', 'Fake breakout có thể — volume thấp hơn bình thường khi breakout = nghi ngờ', 'Volume không quan trọng', 'Nên vào lệnh full size'], c: 1, exp: 'Breakout hợp lệ cần có sự xác nhận của Volume cao hơn mức bình thường. Nếu Volume thấp hơn trung bình, đó rất có thể là một cú Fake Breakout (Phá vỡ giả).' },
    { q: 'MACD Line vừa cắt Signal Line từ trên xuống (Death Cross) khi cả 2 đang ở trên 0. Tín hiệu này là gì?', opts: ['Tín hiệu BUY', 'Tín hiệu SELL — Bearish Death Cross xác nhận momentum chuyển xuống', 'Tín hiệu trung lập', 'MACD không đáng tin'], c: 1, exp: 'Sự giao cắt hướng xuống của đường MACD dưới đường tín hiệu là tín hiệu Death Cross, xác nhận động lượng (momentum) đang chuyển sang hướng giảm.' },
    { q: 'Bạn DCA mua BTC: T1 mua $500 ở $60,000; T2 mua $500 ở $40,000; T3 mua $500 ở $30,000. Giá BTC trung bình bạn mua được gần nhất là bao nhiêu?', opts: ['$43,333 (trung bình cộng đơn giản)', '$40,000 (thực tế thấp hơn trung bình cộng vì mua được nhiều BTC hơn khi giá rẻ)', '$30,000 (giá thấp nhất)', '$60,000 (giá cao nhất)'], c: 1, exp: 'Khi chia đều số tiền đầu tư (DCA), bạn sẽ mua được số lượng tài sản nhiều hơn khi giá rẻ, do đó giá vốn trung bình thực tế sẽ thấp hơn mức trung bình cộng đơn giản của các mốc giá.' },
    { q: 'WMA khác EMA như thế nào?', opts: ['WMA giống SMA hoàn toàn', 'WMA dùng trọng số tuyến tính (1,2,3...N) còn EMA dùng trọng số hàm mũ — cả 2 đều ưu tiên giá gần hơn nhưng khác cách tính', 'WMA chậm hơn SMA', 'EMA và WMA là một'], c: 1, exp: 'Cả WMA và EMA đều ưu tiên dữ liệu giá gần nhất, nhưng WMA tính theo trọng số tuyến tính cộng dồn, trong khi EMA tính theo trọng số hàm mũ, giúp EMA mượt hơn.' },
    { q: 'Giá tăng mạnh nhưng Histogram MACD đang nhỏ dần mỗi ngày. Tín hiệu gì?', opts: ['Xu hướng tăng đang mạnh lên', 'Momentum đang yếu dần dù giá cao hơn — dạng MACD Divergence, cẩn thận đảo chiều', 'MACD bị lỗi kỹ thuật', 'Không có ý nghĩa'], c: 1, exp: 'Khi Histogram MACD thu hẹp lại trong khi giá vẫn tăng, điều đó báo hiệu động lượng tăng đang suy yếu (một dạng phân kỳ), tiềm ẩn rủi ro đảo chiều hoặc điều chỉnh.' },
    { q: 'CHoCH trong uptrend xảy ra khi nào?', opts: ['Giá tạo đỉnh mới cao hơn', 'Giá đóng cửa dưới đáy HL gần nhất — cơ cấu uptrend bị phá lần đầu tiên', 'EMA21 cắt xuống EMA50', 'RSI về dưới 50'], c: 1, exp: 'Change of Character (CHoCH) trong xu hướng tăng xảy ra khi giá phá vỡ và đóng cửa dưới đáy dẫn tới đỉnh cao nhất (HL gần nhất), báo hiệu sự phá vỡ cấu trúc tăng.' },
    { q: 'Bạn muốn vào BUY vàng XAU/USD. Bao nhiêu điều kiện hội tụ sau đây là lý tưởng nhất? (A) D1 uptrend rõ. (B) Giá pullback về EMA21. (C) RSI điều chỉnh về 45-50. (D) Có Hammer tại Fibonacci 62%. (E) Volume tăng nhẹ trên nến xác nhận.', opts: ['Chỉ cần A là đủ', 'A + B là đủ', 'A + B + C + D + E — càng nhiều điều kiện hội tụ, xác suất setup càng cao', 'D + E là quan trọng nhất'], c: 2, exp: 'Giao dịch theo xác suất (Confluence) có nghĩa là càng nhiều công cụ và khung thời gian đồng thuận (hội tụ) cho cùng một hướng, tỷ lệ chiến thắng của lệnh đó càng cao.' }
  ];
    ...
    <div className="mt-10">
      ...
      {!showRes ? (
        ...
        <div className="p-10 bg-gradient-to-br from-white to-gray-50 dark:from-[#0B0E11] dark:to-[#181A20] border border-yellow-500 dark:border-[#FCD535] rounded-3xl text-center shadow-lg mt-8">
          ...
          <p className="text-gray-600 dark:text-[#848E9C] text-[18.5px] mb-6">{score >= 10 ? <><Trophy size={18} className="inline mr-1 text-yellow-500" /> Xuất sắc! Nắm vững Chương 2! Sẵn sàng học Phương pháp NNN.</> : <><Dumbbell size={48} className="mx-auto text-blue-500 mb-4" /> Chưa đạt 10/12. Đọc lại các bài có câu sai, đặc biệt là RSI và S/R.</>}</p>
          <button onClick={() => { setAnswers({}); setShowRes(false); }} className="bg-gray-800 dark:bg-[#2B3139] text-white px-8 py-3 rounded-xl font-bold uppercase text-sm active:bg- md:hover:bg-black transition-colors">Làm lại</button>
        </div>
      )}
    </div>
  );
...
const CHAPTER_2_DATA_VN = [
  {
    ...
    content: (
      <>
        ...
        <p className="text-[18.5px] md:text-[19.5px] leading-[1.8] text-gray-800 dark:text-[#EAECEF] mb-4"><strong>Hỗ trợ (Support)</strong> là vùng giá mà nhiều người mua sẵn sàng mua vào — họ nghĩ "giá thế này là rẻ, mình mua". Lực cầu lớn → giá bật lên.</p>
        <p className="text-[18.5px] md:text-[19.5px] leading-[1.8] text-gray-800 dark:text-[#EAECEF] mb-6"><strong>Kháng cự (Resistance)</strong> là vùng giá mà nhiều người muốn bán ra — họ nghĩ "giá đã lên cao, mình chốt lời". Lực cung lớn → giá bị đẩy xuống.</p>
        <p className="text-[18.5px] md:text-[19.5px] leading-[1.8] text-gray-800 dark:text-[#EAECEF] mb-6">Những vùng này tồn tại vì <em>con người nhớ giá</em>. Ai từng mua BTC ở $50,000 rồi bị kẹt khi giá rơi về $30,000 thường sẽ muốn bán khi giá quay lại $50,000 để thoát hàng. Khi nhiều người cùng nghĩ như vậy, vùng kháng cự hình thành.</p>
        <div className="space-y-4 mb-6">
          <div className="flex gap-4 items-start"><div className="w-8 h-8 rounded-full bg-blue-100 dark:bg-[#378ADD]/20 text-blue-600 dark:text-[#378ADD] font-bold flex items-center justify-center shrink-0">1</div><p className="text-[18.5px] text-gray-800 dark:text-[#EAECEF] pt-1">Kháng cự $2,000 (nhiều người đặt lệnh SELL ở đây). Giá tăng lên $2,000, bị đẩy xuống → <strong>Kháng cự xác nhận.</strong></p></div>
          <div className="flex gap-4 items-start"><div className="w-8 h-8 rounded-full bg-blue-100 dark:bg-[#378ADD]/20 text-blue-600 dark:text-[#378ADD] font-bold flex items-center justify-center shrink-0">2</div><p className="text-[18.5px] text-gray-800 dark:text-[#EAECEF] pt-1">Sau nhiều lần, giá vượt qua $2,000 và <strong>đóng cửa trên đó</strong>. Những người SELL bị thua — họ phải mua lại để cắt lỗ.</p></div>
          <div className="flex gap-4 items-start"><div className="w-8 h-8 rounded-full bg-blue-100 dark:bg-[#378ADD]/20 text-blue-600 dark:text-[#378ADD] font-bold flex items-center justify-center shrink-0">3</div><p className="text-[18.5px] text-gray-800 dark:text-[#EAECEF] pt-1">$2,000 giờ là <strong>hỗ trợ mới</strong>. Nếu giá về lại, những người đã mua sẽ mua thêm, người chờ cơ hội cũng mua vào.</p></div>
        </div>
  ...
  },
  {
    ...
    content: (
      <>
        ...
        <p className="text-[18.5px] md:text-[19.5px] leading-[1.8] text-gray-800 dark:text-[#EAECEF] mb-6">Hai khái niệm này giúp bạn <em>đọc sớm hơn</em> khi xu hướng thay đổi — thay vì chờ đến khi quá rõ ràng (và đã muộn).</p>
        <CyberTable
  ...
  },
  {
    ...
    content: (
      <>
        ...
        <p className="text-[18.5px] md:text-[19.5px] leading-[1.8] text-gray-800 dark:text-[#EAECEF] mb-4">Tưởng tượng bạn theo dõi điểm số của một học sinh qua 5 bài kiểm tra: <strong>60, 70, 65, 80, 90</strong>. Cách bạn tính trung bình sẽ quyết định dữ liệu mới nhất có được ưu tiên hay không.</p>
        <CyberTable
        ...
        <div className="bg-gray-900 dark:bg-[#0A0D13] border border-gray-700 dark:border-[#2B3139] rounded-2xl p-6 my-8">
          <div className="text-[14.5px] text-gray-400 font-mono uppercase tracking-widest mb-2">// Bộ 3 EMA — Thứ tự xếp chồng xác định xu hướng</div>
...
export default CHAPTER_2_DATA_VN;
